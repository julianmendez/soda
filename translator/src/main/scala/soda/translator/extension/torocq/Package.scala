package soda.translator.extension.torocq

/*
 * This package contains classes for the translation to Gallina,
 * the specification language used by Rocq.
 */
    ...
    soda.translator.block.BlockTranslator
{
  ...
  private lazy val _tc = TranslationConstantToRocq .mk
...
case class MicroTranslatorToRocq_ () extends MicroTranslatorToRocq

object MicroTranslatorToRocq {
  def mk : MicroTranslatorToRocq =
    MicroTranslatorToRocq_ ()
}


trait RocqClassAliasBlockTranslator
  extends
    soda.translator.block.BlockTranslator
{
  ...
  private lazy val _tc : TranslationConstantToRocq = TranslationConstantToRocq .mk
  ...
  private def _process_class_alias (line : String) : String =
    line
      .replace (_sc .class_alias_reserved_word + _sc .space , _tc .rocq_notation_prefix)
      .replace (_sc .space + _sc .class_alias_definition_symbol + _sc .space ,
         _tc .rocq_notation_infix) +
      _tc .rocq_notation_suffix
...
case class RocqClassAliasBlockTranslator_ () extends RocqClassAliasBlockTranslator

object RocqClassAliasBlockTranslator {
  def mk : RocqClassAliasBlockTranslator =
    RocqClassAliasBlockTranslator_ ()
}


trait RocqClassConstructorBlockTranslator
  extends
    soda.translator.block.BlockTranslator
{
  ...
  private lazy val _sp : String = _tc .rocq_space
  ...
  private def _translate_type_symbols (line : String) : String =
    line
      .replaceAll (_sc .subtype_reserved_word , _tc .rocq_subtype_symbol)
      .replaceAll (_sc .supertype_reserved_word , _tc .rocq_supertype_symbol)
      .replaceAll (_sc .function_arrow_symbol , _tc .rocq_function_arrow_symbol)
  ...
  private def _get_constructor_declaration (beginning : ClassBeginningAnnotation)
    ...
    _sp +
    _tc .rocq_opening_brace +
    _tc .rocq_new_line +
    _get_initial_spaces (beginning) + _four_spaces +
    functions .mkString (_sp + _tc .rocq_semicolon_symbol + _tc .rocq_new_line +
      _get_initial_spaces (beginning) + _four_spaces) +
    _tc .rocq_new_line +
    _tc .rocq_closing_brace + _sp + _tc .rocq_end_symbol + _tc .rocq_new_line +
    _tc .rocq_new_line +
    _tc .rocq_notation_prefix +
    beginning .class_name + _sc .constructor_suffix +
    _tc .rocq_notation_infix +
    beginning .class_name + _tc .rocq_dot_notation_symbol +
    _sc .default_constructor_function + _tc .rocq_notation_suffix
...
case class RocqClassConstructorBlockTranslator_ () extends RocqClassConstructorBlockTranslator

object RocqClassConstructorBlockTranslator {
  def mk : RocqClassConstructorBlockTranslator =
    RocqClassConstructorBlockTranslator_ ()
}


trait RocqClassDeclarationBlockTranslator
  extends
    soda.translator.block.BlockTranslator
{
  ...
  private def _process_after_extends (lines : Seq [String] ) : Seq [String] =
    if ( (get_first_line (lines) .trim .nonEmpty)
       ...
       _tc .rocq_import_reserved_word + _tc .rocq_space + line .trim +
       _tc .rocq_space + _tc .rocq_end_symbol)
    else Seq [String] ()
  ...
  lazy val table_translator : Translator =
    TableTranslator_ (
      Seq (Tuple2 (_sc .class_reserved_word, _tc .rocq_class_reserved_word ) )
    )

  private def _process_head_with (class_name : String) (line : String) : Seq [String] =
    Seq [String] (
      _tc .rocq_module_reserved_word + _tc .rocq_space + class_name + _tc .rocq_space +
      _tc .rocq_end_symbol + _tc .rocq_new_line + _tc .rocq_new_line +
      Replacement_ (_sc .space + line)
        .replace_at_beginning (0) (table_translator)
        .line .substring (_sc .space .length) +
      _tc .rocq_space + _tc .rocq_type_membership_symbol + _tc .rocq_space +
      _tc .rocq_type_reserved_word + _tc .rocq_space + _tc .rocq_function_definition_symbol
    )
...
case class RocqClassDeclarationBlockTranslator_ () extends RocqClassDeclarationBlockTranslator

object RocqClassDeclarationBlockTranslator {
  def mk : RocqClassDeclarationBlockTranslator =
    RocqClassDeclarationBlockTranslator_ ()
}


trait RocqClassEndBlockTranslator
  extends
    soda.translator.block.BlockTranslator
{
  ...
  private def _translate_block_with_abstract_beginning (beginning : ClassBeginningAnnotation)
    ...
    ClassEndAnnotation .mk (
      BlockBuilder .mk .build (
        Seq [String] (
          _tc .rocq_module_end_reserved_word + _tc .rocq_space + beginning .class_name +
          _tc .rocq_space + _tc .rocq_end_symbol ,
          "",
          _tc .rocq_import_reserved_word + _tc .rocq_space + beginning .class_name +
          _tc .rocq_space + _tc .rocq_end_symbol
        )
      ) ) (
      block .references
...
case class RocqClassEndBlockTranslator_ () extends RocqClassEndBlockTranslator

object RocqClassEndBlockTranslator {
  def mk : RocqClassEndBlockTranslator =
    RocqClassEndBlockTranslator_ ()
}


trait RocqDatatypeDeclarationBlockTranslator
  extends
    soda.translator.block.BlockTranslator
{
  ...
  private def _add_final_dot (lines : Seq [AnnotatedLine] ) : Seq [AnnotatedLine] =
    lines .++ (Seq [AnnotatedLine] () .+: (
        AnnotatedLine .mk (_tc .rocq_dot_notation_symbol) (false) ) )
  ...
  private def _get_type_parameters_with (parameters : Seq [String] ) : String =
    if ( parameters .isEmpty
      ...
      parameters
        .map ( param => _tc .rocq_space + _tc .rocq_opening_parenthesis +
          param + _tc .rocq_space + _tc .rocq_type_membership_symbol +
          _tc .rocq_space + _tc .rocq_type_reserved_word  + _tc .rocq_closing_parenthesis)
        .mkString
  ...
  private def _translate_parameter (parameter : String) : String =
    parameter
      .replace (_sc .opening_bracket_symbol , _tc .rocq_opening_parenthesis)
      .replace (_sc .closing_bracket_symbol , _tc .rocq_closing_parenthesis)

  private def _translate_constructors (block : DatatypeDeclarationAnnotation)
      : Seq [AnnotatedLine] =
    block .constructors
      .map ( constr =>
          _tc .rocq_space + _tc .rocq_space + _tc .rocq_vertical_bar_symbol + _tc .rocq_space +
          constr .name +  _tc .rocq_space + _tc .rocq_type_membership_symbol + _tc .rocq_space +
          _parameters_without_variables (constr .parameters)
            .map ( param => _translate_parameter (param) )
            .mkString (_tc .rocq_space + _tc .rocq_function_arrow_symbol + _tc .rocq_space) )
  ...
  def get_type_declaration (block : DatatypeDeclarationAnnotation) : AnnotatedLine =
    AnnotatedLine .mk (
      _tc .rocq_inductive_reserved_word + _tc .rocq_space +
      block .class_name + get_type_parameters (block) + _tc .rocq_space +
      _tc .rocq_type_membership_symbol + _tc .rocq_space +
      _tc .rocq_type_reserved_word + _tc .rocq_space +
      _tc .rocq_function_definition_symbol
    ) (false)
...
case class RocqDatatypeDeclarationBlockTranslator_ () extends RocqDatatypeDeclarationBlockTranslator

object RocqDatatypeDeclarationBlockTranslator {
  def mk : RocqDatatypeDeclarationBlockTranslator =
    RocqDatatypeDeclarationBlockTranslator_ ()
}


trait RocqDirectiveBlockTranslator
  extends
    soda.translator.blocktr.DirectiveBlockTranslator
{
  ...
  lazy val identifier : String = _tc .rocq_directive_identifier

  lazy val opening_comment : String = _tc .rocq_comment_opening_symbol

  lazy val closing_comment : String = _tc .rocq_comment_closing_symbol

}

case class RocqDirectiveBlockTranslator_ () extends RocqDirectiveBlockTranslator

object RocqDirectiveBlockTranslator {
  def mk : RocqDirectiveBlockTranslator =
    RocqDirectiveBlockTranslator_ ()
}


trait RocqDocumentationBlockTranslator
  extends
    soda.translator.block.BlockTranslator
{
  ...
  private def _translate_with_symbol (opening_symbol : String) (lines : Seq [String] ) : Seq [String] =
    _append (
      _tc .rocq_comment_closing_symbol) (
  ...
  private def _get_opening_symbol (line : String) : String =
    ...
    ) _tc .rocq_opening_documentation
    else _tc .rocq_comment_opening_symbol
...
case class RocqDocumentationBlockTranslator_ () extends RocqDocumentationBlockTranslator

object RocqDocumentationBlockTranslator {
  def mk : RocqDocumentationBlockTranslator =
    RocqDocumentationBlockTranslator_ ()
}


trait RocqDotNotationBlockTranslator
  extends
...
case class RocqDotNotationBlockTranslator_ () extends RocqDotNotationBlockTranslator

object RocqDotNotationBlockTranslator {
  def mk : RocqDotNotationBlockTranslator =
    RocqDotNotationBlockTranslator_ ()
}


trait RocqFunctionDefinitionBlockTranslator
  extends
    soda.translator.block.BlockTranslator
{
  ...
  private def _translate_non_recursive_definition (block : FunctionDefinitionAnnotation) : Block =
    ...
    ) _append (_tc .rocq_definition_end_symbol) (_prepend (_tc .rocq_definition_reserved_word +
  ...
  private def _translate_block_with (block : FunctionDefinitionAnnotation) : Block =
    ...
    ) _append (_tc .rocq_recursive_definition_end_symbol) (
...
case class RocqFunctionDefinitionBlockTranslator_ () extends RocqFunctionDefinitionBlockTranslator

object RocqFunctionDefinitionBlockTranslator {
  def mk : RocqFunctionDefinitionBlockTranslator =
    RocqFunctionDefinitionBlockTranslator_ ()
}


trait RocqImportDeclarationBlockTranslator
  extends
    soda.translator.block.BlockTranslator
{
  ...
  lazy val rocq_import_declaration_pattern =
    _tc .rocq_import_reserved_word + _tc .rocq_space
...
case class RocqImportDeclarationBlockTranslator_ () extends RocqImportDeclarationBlockTranslator

object RocqImportDeclarationBlockTranslator {
  def mk : RocqImportDeclarationBlockTranslator =
    RocqImportDeclarationBlockTranslator_ ()
}


trait RocqMatchCaseBlockTranslator
  extends
    soda.translator.block.BlockTranslator
{
  ...
  private def _append_with_after_match (line : String) : String =
    ...
    ) line + _tc .rocq_space + _tc .rocq_with_reserved_word
    else line
  ...
  private def _replace_case (line : String) : String =
    if ( _is_a_case_line (line)
      ...
      ReplacementAux .mk
        . replace_first (line) (_soda_case_pattern) (_tc .rocq_case_translation)
    else line
  ...
  private def _translate_match_case_structure (block: AnnotatedBlock) (tabulation : String) : Block =
    BlockBuilder .mk .build (
      block .lines
        ...
        .++ (Seq [String] () .+: (tabulation + _tc .rocq_match_end_translation) )
    )
...
case class RocqMatchCaseBlockTranslator_ () extends RocqMatchCaseBlockTranslator

object RocqMatchCaseBlockTranslator {
  def mk : RocqMatchCaseBlockTranslator =
    RocqMatchCaseBlockTranslator_ ()
}


trait RocqPackageDeclarationBlockTranslator
  extends
    soda.translator.block.BlockTranslator
{
...
case class RocqPackageDeclarationBlockTranslator_ () extends RocqPackageDeclarationBlockTranslator

object RocqPackageDeclarationBlockTranslator {
  def mk : RocqPackageDeclarationBlockTranslator =
    RocqPackageDeclarationBlockTranslator_ ()
}


trait RocqTheoremBlockTranslator
  extends
    soda.translator.block.BlockTranslator
{
  ...
  private def _translate_block (block : TheoremBlockAnnotation) : TheoremBlockAnnotation =
    TheoremBlockAnnotation .mk (
      _append (
        _tc .rocq_theorem_end_symbol) (_prepend (
          _tc .rocq_theorem_begin_reserved_word) (_remove_first_line (block)
        )
      )
    )
...
case class RocqTheoremBlockTranslator_ () extends RocqTheoremBlockTranslator

object RocqTheoremBlockTranslator {
  def mk : RocqTheoremBlockTranslator =
    RocqTheoremBlockTranslator_ ()
}
...
trait TranslationConstantToRocq
{
  ...
  lazy val rocq_new_line = "\n"

  lazy val rocq_function_definition_symbol = ":="

  lazy val rocq_type_membership_symbol = ":"

  lazy val rocq_subtype_symbol = "<:"

  lazy val rocq_supertype_symbol = ">:"

  lazy val rocq_function_arrow_symbol = "->"

  lazy val rocq_empty_string = ""

  lazy val rocq_vertical_bar_symbol = "|"

  lazy val rocq_match_end_translation = "end"

  lazy val rocq_opening_parenthesis = "("

  lazy val rocq_closing_parenthesis = ")"

  lazy val rocq_comment_opening_symbol = "(*"

  lazy val rocq_comment_closing_symbol = "*)"

  lazy val rocq_opening_documentation = "(**"

  lazy val rocq_closing_documentation = "*)"

  lazy val rocq_some_variable_name = "x"

  lazy val rocq_opening_brace = "{"

  lazy val rocq_closing_brace = "}"

  lazy val rocq_semicolon_symbol = ";"

  lazy val rocq_dot_notation_symbol = "."

  lazy val rocq_product_type_symbol = "*"

  lazy val rocq_lambda_reserved_word = "fun"

  lazy val rocq_lambda_arrow_symbol = "=>"

  lazy val rocq_case_arrow_symbol = "=>"

  lazy val rocq_quotes_symbol = "\""

  lazy val rocq_apostrophe_symbol = "'"

  lazy val rocq_equals_symbol = "=?"

  lazy val rocq_less_than_symbol = "<?"

  lazy val rocq_less_than_or_equal_to_symbol = "<=?"

  lazy val rocq_case_translation = rocq_vertical_bar_symbol + rocq_space

  lazy val rocq_not_reserved_word = "negb"

  lazy val rocq_and_reserved_word = "andb"

  lazy val rocq_or_reserved_word = "orb"

  lazy val rocq_end_symbol = "."

  lazy val rocq_class_reserved_word : String = "Class"

  lazy val rocq_definition_reserved_word : String = "Definition"

  lazy val rocq_inductive_reserved_word : String = "Inductive"

  lazy val rocq_set_reserved_word : String = "Set"

  lazy val rocq_type_reserved_word : String = "Type"

  lazy val rocq_module_reserved_word : String = "Module"

  lazy val rocq_module_end_reserved_word : String = "End"

  lazy val rocq_import_reserved_word : String = "Import"

  lazy val rocq_notation_reserved_word : String = "Notation"

  lazy val rocq_recursive_definition_reserved_word : String = "Fixpoint"

  lazy val rocq_inductive_end_symbol : String = rocq_end_symbol

  lazy val rocq_definition_end_symbol : String = rocq_end_symbol

  lazy val rocq_recursive_definition_end_symbol : String = rocq_end_symbol

  lazy val rocq_with_reserved_word : String = "with"

  lazy val rocq_theorem_begin_reserved_word : String = "Theorem"

  lazy val rocq_theorem_end_symbol : String = rocq_end_symbol

  lazy val rocq_directive_identifier : String = "rocq"

  lazy val rocq_directive_alternative_identifier : String = "coq"

  lazy val rocq_notation_at_level_99 : String = "(at level 99)"

  lazy val rocq_notation_prefix : String =
    rocq_notation_reserved_word + rocq_space + rocq_quotes_symbol + rocq_apostrophe_symbol

  lazy val rocq_notation_infix : String =
     rocq_apostrophe_symbol + rocq_quotes_symbol + rocq_space + rocq_function_definition_symbol +
     rocq_space

  lazy val rocq_notation_suffix : String =
    rocq_space + rocq_notation_at_level_99 + rocq_space + rocq_end_symbol

  lazy val rocq_prelude : Seq [String] =
    Seq (
      "",
  ...
  lazy val rocq_recursive_function_prefixes : Seq [String] =
    Seq (
      "rec_",
      "_rec_",
      "tailrec_",
  ...
  lazy val rocq_reserved_words =
    rocq_1 .++ (rocq_2 .++ (rocq_3 .++ (rocq_4) ) )

  lazy val rocq_1 : Seq [String] =
    Seq (
      "as",
      "else",
      "end",
  ...
  lazy val prefix_rocq_non_soda : String = "__soda__"

  lazy val rocq_non_soda : Seq [Tuple2 [String, String] ] =
    rocq_reserved_words
      .filter ( x => ! soda_constant .soda_reserved_words .contains (x))
      .map ( x => Tuple2 (x , prefix_rocq_non_soda + x) )

  def is_rocq_word (word : String) : Boolean =
    rocq_reserved_words .contains (word)

}

case class TranslationConstantToRocq_ () extends TranslationConstantToRocq

object TranslationConstantToRocq {
  def mk : TranslationConstantToRocq =
    TranslationConstantToRocq_ ()
}
...
/**
 * This translates Soda source code to Rocq source code.
 */

trait TranslatorToRocq
  extends
    soda.translator.extension.common.Extension
{
  ...
  private lazy val _rocq_extension : String = ".v"
  ...
  private def _get_input_output_file_names (input_name : String) : FileNamePair =
    if ( input_name .endsWith (_soda_extension)
      ...
      input_name .substring (0, input_name .length - _soda_extension .length) + _rocq_extension)
    else FileNamePair .mk (input_name + _soda_extension) (input_name + _rocq_extension)
...
case class TranslatorToRocq_ () extends TranslatorToRocq

object TranslatorToRocq {
  def mk : TranslatorToRocq =
    TranslatorToRocq_ ()
}

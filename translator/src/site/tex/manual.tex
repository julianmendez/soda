...
\lstdefinelanguage{Scopus}{
    morekeywords={if, then, else, class, has, extends, with, this, false, true, not, and, or, package, import, new, is, in, to, suchthat, @override, @tailrec, @main},
...
\begin{document}
  ...
  class RegisteredPerson (firstName: String, lastName: String) {
    ...
    It is important to observe that the syntax in \Scopus does not restrict the definition of constants in a class that depends from a constructor from the same class.
    Since the evaluation of a constant occurs at the moment of instantiation, trying to evaluate a constant that uses an instance of the same class produces an infinite loop.
    The solution to this situation is to use a function without parameters instead of a constant.

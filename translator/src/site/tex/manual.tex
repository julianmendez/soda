...
\lstdefinelanguage{Scopus}{
    morekeywords={if, then, else, class, has, extends, with, this, false, true, not, and, or, package, import, new, is, in, suchthat, @tailrec, @override, @private, @main},
...
\begin{document}
    ...
    While in Scala, the use of parameters is expected to denote side effects, in \Scopus it only indicates the delation in its evaluation.
    Thus, a constants is evaluated at the moment a class is instantiated, while the function without parameters is evaluated when it is called by another function.
    This notation could be understood as a \textit{lazy evaluation} of a constant.

    In \Scopus, it is possible to define a function that needs auxiliary computations.
    This can be done by defining a block between braces (\srccode{\{ \}}).
    In this block, all the auxiliary computations are in sequence, and the last line is the result.

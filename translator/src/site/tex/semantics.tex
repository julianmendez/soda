...
We define the class in class definition as a syntactic variant of defining the class outside, but accessing with scope.
...
\]
\[\sclass\ B (V_{A} + V_{B}) \ \sextends\ P_{2} = S_{A} \cup S_{B}
\]

In practice, this is a little more complicated since this would involve a renaming and access the class names using points.
...
\begin{itemize}
    ...
    \item $\I{(\sclass\ A (V_{A}) \ \sextends\ P = S_{A})}$ = \\
    $\I{(\sclass\ A (V_{A}) = S_{P} \cup S_{A})}$, provided that $S_{P}$ and $S_{A}$ do not share any definition of the same function name or class name.

    \item $\I{(\sclass\ A (V_{A}) \ \sextends\ P_{1} \ \swith\ P_{2} = S_{A})}$ = \\
    $\I{(\sclass\ A (V_{A}) \ \sextends\ P_{1} = S_{P_{2}} \cup S_{A})}$, provided that $S_{P_{2}}$ and $S_{A}$ do not share any definition of the same function name or class name.

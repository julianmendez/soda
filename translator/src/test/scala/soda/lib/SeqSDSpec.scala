...
case class SeqSDSpec () extends AnyFunSuite {
  ...
  test ("should detect an empty sequence") {
    ...
    lazy val obtained = seqsd.open (ifEmpty = true, ifNonEmpty = nonEmpty => false
    )
    assert (obtained )
  }
  ...
  test ("should detect an non empty sequence") {
    ...
    lazy val seqsd = SeqSDBuilder () .build (input )
    lazy val obtained = seqsd.open (ifEmpty = false, ifNonEmpty = nonEmpty => true
    )
    assert (obtained )
  }
  ...
  test ("should get the maximum") {
    ...
    lazy val obtained =
      SeqSDBuilder () .build (input ) .open (ifEmpty = NoneSD (), ifNonEmpty = sequence => SomeSD (max (sequence )  )
      )
  ...
  test ("should reverse another sequence") {
    lazy val input = SeqSDBuilder () .build (Seq (2, 7, 1, 8, 2, 8, 1, 8, 2, 8, 4, 5, 9 ) )
    lazy val expected = SeqSDBuilder () .build (Seq (9, 5, 4, 8, 2, 8, 1, 8, 2, 8, 1, 7, 2 ) )
    lazy val obtained = input.open (ifEmpty = EmptySeqSD (), ifNonEmpty = sequence => sequence.reverse
    )
    assert (obtained == expected )
  }

...
case class SeqSDSpec () extends org.scalatest.funsuite.AnyFunSuite {
  ...
  test ("should get the maximum") {
    ...
    lazy val obtained =
      SeqSDBuilder () .build (input ) .opt (ifEmpty = NoneSD (), ifNonEmpty = sequence => SomeSD (max (sequence )  )
      )
  ...
  test ("should reverse another sequence") {
    lazy val input = SeqSDBuilder () .build (Seq (2, 7, 1, 8, 2, 8, 1, 8, 2, 8, 4, 5, 9 ) )
    lazy val expected = SeqSDBuilder () .build (Seq (9, 5, 4, 8, 2, 8, 1, 8, 2, 8, 1, 7, 2 ) )
    lazy val obtained = input.opt (ifEmpty = EmptySeqSD (), ifNonEmpty = sequence => sequence.reverse
    )
    assert (obtained == expected )
  }
}

...
case class CombSpec () extends AnyFunSuite {
  ...
  test ("simple case of Cartesian product of two sequences") {
  ...
  test ("simple case of Cartesian product of empty sequences") {
    lazy val expected = Seq ()
    lazy val obtained = Comb () .cartesianProduct (Seq ()  )
    assert (obtained == expected )
  }

  test ("example of Cartesian product of three sequences") {
    lazy val expected = Seq (

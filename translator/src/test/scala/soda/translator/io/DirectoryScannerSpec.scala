...
trait DirectoryScannerSpec  extends org.scalatest.funsuite.AnyFunSuite {
  import java.io.File
  ...
  test ("simple scan of all files") {
    lazy val scanner = DirectoryScanner_ ()
    lazy val expected = Seq ("otherexample", "Example.md", "Example.scala", "Example.soda", "otherexample/OtherExample.scala", "otherexample/OtherExample.soda", "otherexample/OtherExample.txt"    ) .map (x => new File (start, x )  ) .toSet
...
case class DirectoryScannerSpec_ () extends DirectoryScannerSpec

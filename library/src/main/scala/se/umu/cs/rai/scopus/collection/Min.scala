...
case class MSeqTranslator() {
  ...
  @tailrec final
  def asMSeqRec(seq: Seq[Elem], ms: MSeq): MSeq =
    if ( seq.isEmpty
    ) ms
    else asMSeqRec(seq.tail, Min().prepended(ms, seq.head))

  @tailrec final
  def asSeqRec(ms: MSeq, seq: Seq[Elem]): Seq[Elem] =
    if ( Min().isEmpty(ms)
    ) seq
...
case class Min() {
   ...
   * </pre>
   */
  def foldLeft0(s0: MSeq, s1: MSeq, f: ((MSeq, Elem) => MSeq)): MSeq =
    foldLeftRec(s0, f, s1)

  //

  @tailrec final
  def reverseRec(s0: MSeq, s1: MSeq): MSeq =
    if ( isEmpty(s0)
    ) s1
    else reverseRec(tail(s0), prepended(s1, head(s0)))

  @tailrec final
  def lengthRec(s: MSeq, n: Int): Int =
    if ( isEmpty(s)
    ) n
    else lengthRec(tail(s), n + 1)

  @tailrec final
  def indexOfRec(s: MSeq, e: Elem, n: Int): Int =
    if ( isEmpty(s)
    ) -1
    else if ( head(s) == e
    ) n
  ...
  @tailrec final
  def containsRec(s: MSeq, e: Elem): Boolean =
    if ( isEmpty(s)
    ) false
    else if ( head(s) == e
    ) true
  ...
  @tailrec final
  def takeRevRec(s0: MSeq, n: Int, s1: MSeq): MSeq =
    if ( isEmpty(s0) || n <= 0
    ) s1
    else takeRevRec(tail(s0), n - 1, prepended(s1, head(s0)))

  @tailrec final
  def dropRec(s: MSeq, n: Int): MSeq =
    if ( isEmpty(s) || n <= 0
    ) s
    else dropRec(tail(s), n - 1)

  @tailrec final
  def spanRevRec(s0: MSeq, p: Elem => Boolean, taking: Boolean, s1: MSeq): (MSeq, MSeq) =
  ...
  @tailrec final
  def forallRec(s: MSeq, p: Elem => Boolean): Boolean =
    if ( isEmpty(s)
    ) true
    else if ( ! p(head(s))
    ) false
  ...
  @tailrec final
  def existsRec(s: MSeq, p: Elem => Boolean): Boolean =
    if ( isEmpty(s)
    ) false
    else if ( p(head(s))
    ) true
  ...
  @tailrec final
  def findRec(s: MSeq, p: Elem => Boolean): Option[Elem] =
    if ( isEmpty(s)
    ) None
    else if ( p(head(s))
    ) Some(head(s))
  ...
  @tailrec final
  def filterRevRec(s0: MSeq, f: Elem => Boolean, s1: MSeq): MSeq =
    if ( isEmpty(s0)
    ) s1
    else {
      val newS1 =
  ...
  @tailrec final
  def mapRevRec(s0: MSeq, f: Elem => Elem, s1: MSeq): MSeq =
    if ( isEmpty(s0)
    ) s1
    else mapRevRec(tail(s0), f, prepended(s1, f(head(s0))))

  @tailrec final
  def foldLeftRec(s0: MSeq, f: (MSeq, Elem) => MSeq, s1: MSeq): MSeq =
    if ( isEmpty(s0)
    ) s1

...
It is also recommended to have small abstract classes.
...
In addition, a parameter name should always be a nound, even if the signature is of a function.
...
\begin{tabular}{lll}
    \srccode{LocalTranslator}                         & class               & noun that starts with a capital letter and uses camel case                                       \\
    \srccode{first\_name}                             & constant            & noun that starts with a lowercase letter and uses snake case                                     \\
    \srccode{translate\_paragraph(s: String)}         & function            & predicate that starts with lowercase letter and uses snake case                                  \\
    \srccode{is\_empty}                               & constant            & predicate "is-" that starts with lowercase letter and uses snake case                            \\
    \srccode{as\_translator}                          & constant            & predicate "as-" that starts with lowercase letter and uses snake case                            \\
    \srccode{to\_global\_translator}                  & constant            & predicate "to-" that starts with lowercase letter and uses snake case                            \\
    \srccode{toGlobalTranslator}                      & constant            & predicate "to-" that starts with lowercase letter and uses camel case to be used by JVM projects \\
    \srccode{LocalTranslator\_}                       & concrete class      & noun that starts with a capital letter and uses camel case and ends with an underscore           \\
    \srccode{\_LocalModel}                            & non-public class    & noun that starts with an underscore and then capital letter and uses camel case                  \\
    \srccode{\_internal\_identifier}                  & non-public constant & noun that starts with an underscore and then a lowercase letter and uses snake case              \\
    \srccode{\_find\_internal\_identifier(s: String)} & non-public function & predicate that starts with an underscore and then a lowercase letter and uses snake case         \\
    \srccode{rec\_find\_elem(s: String)}              & recursive function  & predicate that starts with prefix "rec" and then lowercase letter and uses snake case            \\
\end{tabular}

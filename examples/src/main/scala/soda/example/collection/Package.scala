...
trait MSeq [A ]
{
  ...
  def opt [B ] (ifEmpty : B) (ifNonEmpty : NESeq [A] => B) : B =
    this match  {
      case NESeq_ (head, tail) => ifNonEmpty (NESeq .mk (head) (tail) )
...
trait MSeqTranslator [A ]
{
  ...
  private lazy val _asMSeq_initial_value : MSeq [A] = Min .mk .empty
...
trait Min [A ]
{
  ...
  private def _aux_next_value_for (tuple : SpanRevFoldTuple [A] ) (neleft : NESeq [A] )
    ...
    if ( new_taking
    ) SpanRevFoldTuple .mk [A] (neleft .tail) (prepended (tuple .right) (neleft .head) ) (
  ...
  private def _forall_next_value (tuple : ForallExistsFoldTuple [A] ) (elem : A)
    ...
    ForallExistsFoldTuple .mk (tuple .status && tuple .condition (elem) ) (tuple .condition)
  ...
  private def _find_next_value (tuple : FindFoldTuple [A] ) (elem : A) : FindFoldTuple [A] =
    ...
    ) FindFoldTuple .mk (SomeSD .mk [A] (elem) ) (tuple .condition)

...
trait NonEmptySortedSequence [A <: Comparable [A]]  extends SortedSequence [A] {
  ...
  lazy val sequence =
    {
      lazy val first_part =
        _remaining.takeWhile (x => is_less_than (x, _element )  )

      lazy val middle = Seq (_element )

      lazy val last_part =
        _remaining.dropWhile (x => is_less_than (x, _element )  )
      first_part.++ (middle.++ (last_part )  ) }
}

case class _NonEmptySortedSequence_ [A <: Comparable [A]]  (_element: A, _remaining: Seq [A]  )  extends NonEmptySortedSequence [A]
...
trait SortedSequenceBuilder [A <: Comparable [A]] {
  ...
  def _next_value_function (sorted_sequence: SortedSequence [A], element: A ): SortedSequence [A] =
    _NonEmptySortedSequence_ [A]  (_remaining = sorted_sequence.sequence, _element = element )
}
